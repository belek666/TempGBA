...
#define HOTKEY_CONTENT_X 156
	...
	char* REASON_NAMES[CODE_CACHE_FLUSH_REASON_COUNT] = {
		"Init", "Full", "Tag", "Link", "ROM", "State"
	};
	...
	void tools_debug_utilisation_menu_passive()
	{
		...
		PRINT_STRING_BG(down_screen_addr, "Current", COLOR_INACTIVE_ITEM, COLOR_TRANS, 85, GUI_ROW1_Y + TEXT_OFFSET_Y);
#ifdef PERFORMANCE_IMPACTING_STATISTICS
		PRINT_STRING_BG(down_screen_addr, "Peak", COLOR_INACTIVE_ITEM, COLOR_TRANS, 170, GUI_ROW1_Y + TEXT_OFFSET_Y);
#endif
		int i;
		for (i = 0; i < TRANSLATION_REGION_COUNT; i++)
		{
			...
			switch (i)
			{
				case TRANSLATION_REGION_IWRAM: Current = iwram_next_code - iwram_code_cache; break;
				case TRANSLATION_REGION_EWRAM: Current = ewram_next_code - ewram_code_cache; break;
				case TRANSLATION_REGION_VRAM:  Current = vram_next_code  - vram_code_cache;  break;
				case TRANSLATION_REGION_ROM:   Current = rom_next_code   - rom_code_cache;   break;
				case TRANSLATION_REGION_BIOS:  Current = bios_next_code  - bios_code_cache;  break;
			}
			sprintf(line_buffer, "%u", Current);
			PRINT_STRING_BG(down_screen_addr, line_buffer, COLOR_INACTIVE_ITEM, COLOR_TRANS, 85, GUI_ROW1_Y + (i + 1) * GUI_ROW_SY + TEXT_OFFSET_Y);
#ifdef PERFORMANCE_IMPACTING_STATISTICS
			sprintf(line_buffer, "%u", Stats.TranslationBytesPeak[i]);
			PRINT_STRING_BG(down_screen_addr, line_buffer, COLOR_INACTIVE_ITEM, COLOR_TRANS, 170, GUI_ROW1_Y + (i + 1) * GUI_ROW_SY + TEXT_OFFSET_Y);
#endif
		}
	}
	...
	void tools_debug_flush_menu_passive()
	{
		...
		u32 reason, cache;
		for (reason = 0; reason < CODE_CACHE_FLUSH_REASON_COUNT; reason++)
			PRINT_STRING_BG(down_screen_addr, REASON_NAMES[reason], COLOR_INACTIVE_ITEM, COLOR_TRANS, OPTION_TEXT_X + (reason + 1) * ((NDS_SCREEN_WIDTH - OPTION_TEXT_X * 2) / (CODE_CACHE_FLUSH_REASON_COUNT + 1)), GUI_ROW1_Y + TEXT_OFFSET_Y);
		for (cache = 0; cache < TRANSLATION_REGION_COUNT; cache++)
		{
			...
			for (reason = 0; reason < CODE_CACHE_FLUSH_REASON_COUNT; reason++)
			{
				u32 x = OPTION_TEXT_X + (reason + 1) * ((NDS_SCREEN_WIDTH - OPTION_TEXT_X * 2) / (CODE_CACHE_FLUSH_REASON_COUNT + 1));
