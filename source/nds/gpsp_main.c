...
void init_main()
{
  ...
  CODE_CACHE_FLUSH_REASON flush_reason = (caches_inited)
  ...
  // FLUSH_REASON_NATIVE_LINK. So init the cache only before the first ROM.
  if (!caches_inited)
    flush_translation_cache(TRANSLATION_REGION_BIOS, FLUSH_REASON_EMULATOR_INIT);
  ...
  flush_translation_cache(TRANSLATION_REGION_VRAM, flush_reason);
