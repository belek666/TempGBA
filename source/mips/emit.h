...
#define mips_emit_special2(function, rs, rt, rd, imm)                         \
  *((u32 *)translation_ptr) = (mips_opcode_special2 << 26) | ((rs) << 21) | ((rt) << 16) | ((rd) << 11) | ((imm) << 6) | mips_special2_##function; \
  translation_ptr += 4;                                                       \
...
#define generate_indirect_branch_no_cycle_update(type)                        \
  mips_emit_j(mips_absolute_offset(mips_indirect_branch_##type##_no_update_gba)); \
  mips_emit_nop();                                                            \
...
#define generate_shift_reg_ror_flags(_rm, _rs)                                \
  generate_load_reg_pc(reg_a1, _rm, 12);                                      \
  generate_load_reg_lower_byte(reg_temp, _rs);                                \
  mips_emit_b(beq, reg_temp, reg_zero, 3);                                    \
  mips_emit_addiu(reg_temp, reg_temp, -1);                                    \
  mips_emit_srlv(reg_temp, reg_a1, reg_temp);                                 \
  mips_emit_andi(reg_c_cache, reg_temp, 1);                                   \
  mips_emit_andi(reg_temp, arm_to_mips_reg[_rs], 0x1f);                       \
  mips_emit_srlv(reg_a0, reg_a1, reg_temp);                                   \
  mips_emit_subu(reg_temp, reg_zero, reg_temp);                               \
...
u32 execute_spsr_restore_body(u32 address)
{
  ...
  if (((io_registers[REG_IE] & io_registers[REG_IF]) != 0) && GBA_IME_STATE && ARM_IRQ_STATE)
  {
    address |= 0x80000000;
  }
...
#define multiply_ticks()                                                      \
{                                                                             \
  generate_load_reg(reg_a0, rs);                                              \
  mips_emit_sra(reg_temp, reg_a0, 31);                                        \
  mips_emit_xor(reg_a0, reg_a0, reg_temp);                                    \
                                                                              \
  mips_emit_srl(reg_temp, reg_a0, 8);                                         \
  mips_emit_b(beq, reg_temp, reg_zero, 8);                                    \
  mips_emit_addiu(reg_rv, reg_zero, 1);                                       \
                                                                              \
  mips_emit_srl(reg_temp, reg_a0, 16);                                        \
  mips_emit_b(beq, reg_temp, reg_zero, 5);                                    \
  mips_emit_addiu(reg_rv, reg_zero, 2);                                       \
                                                                              \
  mips_emit_srl(reg_temp, reg_a0, 24);                                        \
  mips_emit_b(beq, reg_temp, reg_zero, 2);                                    \
  mips_emit_addiu(reg_rv, reg_zero, 3);                                       \
                                                                              \
  mips_emit_addiu(reg_rv, reg_zero, 4);                                       \
                                                                              \
  mips_emit_subu(reg_cycles, reg_cycles, reg_rv);                             \
}                                                                             \
...
u32 execute_store_cpsr_body(u32 _cpsr, u32 store_mask)
{
  if ((store_mask & 0xFF) != 0)
  {
    ...
    if (((io_registers[REG_IE] & io_registers[REG_IF]) != 0) && GBA_IME_STATE && ((_cpsr & 0x80) == 0))
    {
      return 1;
    }
  }
...
{                                                                             \
  ...
  arm_block_memory_address_##access_type(offset_type, writeback_type);        \
                                                                              \
  if ((rn == REG_SP) && (iwram_stack_optimize != 0))                          \
  {                                                                           \
