...
const u8 ALIGN_DATA obj_width_table[] =
{
  8, 16, 32, 64, 16, 32, 32, 64, 8, 8, 16, 32
};

const u8 ALIGN_DATA obj_height_table[] =
{
...
static void render_scanline_conditional_tile(u32 start, u32 end, u16 *scanline, u8 enable_flags, u16 dispcnt, u16 bldcnt, TileLayerRenderStruct *layer_renderers);
static void render_scanline_conditional_bitmap(u32 start, u32 end, u16 *scanline, u8 enable_flags, u16 dispcnt, u16 bldcnt, BitmapLayerRenderStruct *layer_renderers);
...
#define color_combine_mask(layer)                                             \
  ((color_combine_mask_a(layer) | ((io_registers[REG_BLDCNT] >> (layer + 7)) & 0x02)) << 9) \
...
{                                                                             \
  ...
  u16 bg_control = io_registers[REG_BG0CNT + layer];                          \
  u32 map_size = (bg_control >> 14) & 0x03;                                   \
  u32 map_width = map_widths[map_size];                                       \
/*u32 map_height = map_heights[map_size]; */                                  \
  u32 horizontal_offset = (io_registers[REG_BG0HOFS + (layer << 1)] + start) % 512; \
  u32 vertical_offset = (io_registers[REG_VCOUNT] + io_registers[REG_BG0VOFS + (layer << 1)]) % 512; \
  u32 current_pixel;                                                          \
  u32 current_pixels;                                                         \
...
/* u16 vcount = io_registers[REG_VCOUNT]; */                                  \
  ...
  render_scanline_dest_##alpha_op *dest_ptr = ((render_scanline_dest_##alpha_op *)scanline) + start; \
                                                                              \
  dx = *((s16 *)io_registers + (REG_BG2PA + (layer_offset << 3)));            \
  dy = *((s16 *)io_registers + (REG_BG2PC + (layer_offset << 3)));            \
...
#define render_scanline_vram_setup_mode4()                                    \
  u16 *palette = palette_ram;                                                 \
  u8 *src_ptr;                                                                \
  if ((io_registers[REG_DISPCNT] & 0x10) != 0)                                \
  {                                                                           \
    src_ptr = vram + 0xA000;                                                  \
  }                                                                           \
  else                                                                        \
...
{                                                                             \
/* u16 bg_control = io_registers[REG_BG2CNT]; */                              \
  u32 current_pixel;                                                          \
  s32 source_x, source_y;                                                     \
/* u16 vcount = io_registers[REG_VCOUNT]; */                                  \
  s32 pixel_x, pixel_y;                                                       \
...
#define obj_scale_offset_2D(color_depth)                                      \
  tile_ptr = obj_tile_base +                                                  \
             ((obj_tile_number_2D_##color_depth +                             \
               ((vertical_offset / 8) * 1024) +                               \
               ((vertical_offset % 8) * tile_width_##color_depth)) & 0x7FFF)  \
...
#define render_scanline_obj_extra_variables_copy(type)                        \
  u16 bldcnt  = io_registers[REG_BLDCNT];                                     \
  u16 dispcnt = io_registers[REG_DISPCNT];                                    \
  u8 obj_enable = io_registers[REG_WINOUT] >> 8;                              \
...
static void order_layers(u8 layer_flags)
{
  ...
  for (priority = 3; priority >= 0; priority--)
  {
    ...
    {
      if (((layer_flags >> layer_number) & 1) && ((io_registers[REG_BG0CNT + layer_number] & 0x03) == priority))
      {
    ...
    if ((obj_priority_count[priority][io_registers[REG_VCOUNT]] > 0) && ((layer_flags & 0x10) != 0))
    {
...
#define effect_condition_fade(pixel_source)                                   \
  (((pixel_source & 0x00000200) != 0) || (((io_registers[REG_BLDCNT] & 0x0020) != 0) && ((pixel_source & 0x1FF) == 0))) \
...
static void expand_blend(u32 *screen_src_ptr, u16 *screen_dest_ptr, u32 start, u32 end)
{
  ...
  u16 bldalpha = io_registers[REG_BLDALPHA];
...
#define render_condition_alpha                                                \
  (((io_registers[REG_BLDALPHA] & 0x1F1F) != 0x001F) &&                       \
   ((io_registers[REG_BLDCNT] & 0x003F) != 0) &&                              \
   ((io_registers[REG_BLDCNT] & 0x3F00) != 0))                                \

#define render_condition_fade                                                 \
  (((io_registers[REG_BLDY] & 0x1F) != 0) && ((io_registers[REG_BLDCNT] & 0x3F) != 0)) \
...
static void render_scanline_conditional_tile(u32 start, u32 end, u16 *scanline, u8 enable_flags, u16 dispcnt, u16 bldcnt, TileLayerRenderStruct *layer_renderers)
{
  ...
  render_layers_color_effect(render_layers_conditional, (layer_count && (enable_flags & 0x1F)), ((enable_flags & 0x20) && render_condition_alpha), ((enable_flags & 0x20) && render_condition_fade), start, end);
}
...
#define limit_window_horizontal_dimensions(window_number)                     \
  if (window_##window_number##_x1 > 240) window_##window_number##_x1 = 240;   \
  if (window_##window_number##_x2 > 240) window_##window_number##_x2 = 240;   \

#define window_x_coords(window_number)                                        \
  window_##window_number##_x1 = io_registers[REG_WIN##window_number##H] >> 8; \
  window_##window_number##_x2 = io_registers[REG_WIN##window_number##H] & 0xFF; \
...
{                                                                             \
  u16 bldcnt = io_registers[REG_BLDCNT];                                      \
  u16 vcount = io_registers[REG_VCOUNT];                                      \
  u16 winin  = io_registers[REG_WININ];                                       \
  u16 winout = io_registers[REG_WINOUT];                                      \
...
void update_scanline(void)
{
  ...
  u16 dispcnt = io_registers[REG_DISPCNT];
  u16 *screen_offset = GBAScreen + (io_registers[REG_VCOUNT] * GBAScreenPitch);
